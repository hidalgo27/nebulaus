@extends('layouts.default')
@section('title')
    <meta name="description" content="Desarrollamos un branding estratégico para tu empresa, desde la creación y diseño de logos que comuniquen la identidad de tu marca."/>
    <title>Branding y diseño de marca</title>
@endsection
@section('content')
	<div class="stunning-header bg-branding">
		<div class="stunning-header-content">
			<h1 class="stunning-header-title">BRANDING</h1>
			<ul class="breadcrumbs">
				<li class="breadcrumbs-item">
					<a href="/">Home</a>
					<i class="seoicon-right-arrow"></i>
					<a href="/#servicios">Servicios</a>
					<i class="seoicon-right-arrow"></i>
				</li>
				<li class="breadcrumbs-item active">
					<span href="#">Branding</span>
					<i class="seoicon-right-arrow"></i>
				</li>
			</ul>
		</div>
	</div>
    <!--CREACION DE MARCAS-->
	<div class="container-fluid ">
        <div class="container medium-padding80">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h2 class="h3 heading-title c-brand-p">Creación de marcas e imagen empresarial</h2>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/branding.png" alt="creacion-de-marca">
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p>Escuchamos tus ideas, nos encargamos de crear y actualizar tu identidad, definir la esencia de la marca y transmitir el mensaje correcto para generar un mayor impacto y tener presencia.</p>
                            <p>¡Que tu marca se comunique por sí sola!</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpurFSrTfr/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpvwFKrv13/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpu5ZALFs_/">
                            </blockquote>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--DISEÑO DE LOGOTIPO-->
    <div class="container-fluid">
        <div class="container">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h2 class="h3 heading-title c-brand-p">Diseño de logotipo</h2>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p>Un logo es el primer paso para posicionar tu marca. Además, te permite diferenciarte de tu competencia.</p>
                            <p>No creemos en plantillas, trabajamos en equipo para que tu logo refleje la esencia de tu marca y sea realmente único.</p>
                        </div>
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/diseño-logotipo.png" alt="diseño-logotipo">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container">
            <div class="row">
                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
					<div class="features-item">
						<blockquote class="instagram-media instagram"
							data-instgrm-permalink="https://www.instagram.com/p/CNpvm9-LI8d/">
						</blockquote>
					</div>
				</div>
                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
					<div class="features-item">
						<blockquote class="instagram-media instagram"
							data-instgrm-permalink="https://www.instagram.com/p/CNpwF9qrD-W/">
						</blockquote>
					</div>
				</div>
                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
					<div class="features-item">
						<blockquote class="instagram-media instagram"
							data-instgrm-permalink="https://www.instagram.com/p/CNpwVQ-r5XX/">
						</blockquote>
					</div>
				</div>
            </div>
        </div>
    </div>
    <!--MANUAL DE MARCA-->
    <div class="container-fluid">
        <div class="container medium-padding80">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h3 class="c-brand-p">Manual de marca</h3>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/manual-de-marca.png" alt="manual-de-marca">
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify pt60">
                            <br><p>Un manual de marca tiene como premisa fundamental decir cómo debe ser usada una marca.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <img loading="lazy" src="img/branding/manual-marca.jpg" alt="manual de marca">
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <img loading="lazy" src="img/branding/manual-marca3.jpg" alt="manual de marca">
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <img loading="lazy" src="img/branding/manual-marca2.jpg" alt="manual de marca">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--IDENTIDAD COORPORATIVA-->
    <div class="container-fluid">
        <div class="container">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h3 class="c-brand-p">Identidad corporativa</h3>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p><br>La Identidad Corporativa es la personalidad de la empresa. Su forma de comunicar, tanto con los clientes como con los empleados, además de su representación e imagen gráfica en todos los ámbitos (logo, papelería, web corporativa, reds, blog,…).</p>
                        </div>
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/identidad.png" alt="identidad-corporativa">
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="ccol-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpv9v3LFDZ/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpvt0aLTDO/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpwTc8LMho/">
                            </blockquote>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--PAPELERIA COORPORATIVA-->
	<div class="container-fluid ">
        <div class="container medium-padding80">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h3 class="c-brand-p">Papelería corporativa</h3>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/papeleria-coorporativa.png" alt="papeleria-corporativa">
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p>
                                <br>Tarjetas de negocio, sobre y hoja carta. Su identidad visual será profesionalmente aplicada en sus piezas de comunicación más habituales.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="ccol-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpuyr7L0Mc/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpvlZ3Lh-S/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpwDGCLKZJ/">
                            </blockquote>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--DISEÑO GRAFICO-->
    <div class="container-fluid">
        <div class="container">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h3 class="c-brand-p">Diseño gráfico</h3>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
				</div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p>
                                <br>El objetivo no es conquistar visualmente la mente del consumidor, sino también su corazón
                            </p>
                        </div>
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <img loading="lazy" src="img/branding/diseño-grafico.png" alt="diseño-grafico">
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpue3erL_7/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpv2EoLM4n/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpvpXYrBBX/">
                            </blockquote>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--FOTOGRAFÍA PUBLICITARIA-->
    <div class="container-fluid">
        <div class="container medium-padding80">
            <div class="container pb30">
                <div class="row pb30">
                    <div class="col-lg-12">
                        <div class="heading align-center">
                            <h3 class="c-brand-p">Fotografía publicitaria</h3>
                            <div class="heading-line">
                                <span class="short-line bg-secondary1-brand"></span>
                                <span class="long-line bg-secondary1-brand"></span>
                            </div>
                        </div>
                    </div>
				</div>
                <div class="row">
                    <div class="col-lg-10 col-lg-offset-1">
                        <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 align-center">
                            <br><img loading="lazy" src="img/branding/fotografia.png" alt="fotografia-publicitaria">
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 c-brand-s3 align-justify">
                            <p>
                                Por medio de un enfoque adecuado, por un reflejo preciso, por un ángulo de visión y por un control de la luz, las fotografías publicitarias logran poner un producto en valor.
                                <br>La fotografía ha dado un giro muy importante en pleno siglo XXl, Los vemos en las redes sociales, banners publicitarios con un solo propósito, de transmitirnos un mensaje de valor detrás de cada detalle. Ahí está la magia de la fotografía publicitaria.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container pb80">
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpwbGnrPhO/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpu7mbLw5g/">
                            </blockquote>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="features-item">
                            <blockquote class="instagram-media instagram"
                                data-instgrm-permalink="https://www.instagram.com/p/CNpuuN4rIr5/">
                            </blockquote>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
